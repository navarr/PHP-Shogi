...
<?php
	if(isset($_GET["test"])) { $get = intval($_GET["test"]); } else { $get = 0; }
?>
<a href="./test.php?test=1">First Test</a> &middot;
<?php if($get-1 > 0) { ?><a href="./test.php?test=<?= $get-1 ?>">Previous Test</a> &middot; <?php } ?>
<?php if($get < count($tests)) { ?><a href="./test.php?test=<?= $get+1 ?>">Next Test</a> &middot; <?php } ?>
<?php if($get != 0) { ?><a href="./test.php?test=0">All Tests</a><?php } else { ?>All Tests<?php } ?>
<br /><br />
<table class="results" summary="">
	<tr><th>Test #</th><th>Description</th><!-- <th>Turn</th> --><th>Type</th><th>Result</th><th>Log</th></tr>
<?php
	if($get == 0) { $get = count($tests); }
